...
def read_data(dataset_name, attr, seed) -> tuple[Dataset, Dataset, list, list]:

    dataset_orig, privileged_groups,unprivileged_groups, pos_class, label_name = get_data(dataset_name,attr)
    df_train, df_test = train_test_split(dataset_orig, test_size=0.3, shuffle=True)
    ...
    # df_train = BinaryLabelDataset(favorable_label=1, unfavorable_label=0, df=df_train, label_names=['Probability'],
    #                             protected_attribute_names=[attr])
    
    # df_test = BinaryLabelDataset(favorable_label=1, unfavorable_label=0, df=df_test,
...
def get_fairness_thresholds(dataset, attr=None, metric=None):
    ...
    return filtered_exp[metric].min().abs()
