...
def measure_train_score_testing(chrom, dataset_orig, data_orig_with_predictions, privileged_group, unprivileged_group):
    ...
    spd = round(abs(metrics.statistical_parity(dataset_orig.privileged_groups)),3)
    aod = round(abs(metrics.average_odds(dataset_orig.privileged_groups)),3)
    eod = round(abs(metrics.equal_opportunity(dataset_orig.privileged_groups)),3)
    # spd, aod, eod = 0.2
...
def run_models_testing(chrom, train_set, test_set):
    ...
    for model,mutation in zip(chrom.__dict__['model_list'], chrom.__dict__['mutation_list']):
        if model.__dict__['is_on'] == True:
            ensemble_estimators_list.append((model.__dict__['name'], model.__dict__['ml_model']))
            mutation_list.append(mutation)
    
    for model, mutation in zip(ensemble_estimators_list, mutation_list):
        df_train = mutate_df(train_set, mutation)
    ...
    pred = ensemble_model.predict(test_set.features)
