...
class MyTrainingEvaluation():
    ...
    def measure_train_score(self, chrom, val_df: pd.DataFrame, val_df_predictions: pd.DataFrame):
        ...
        spd_list = [round(abs(metrics.statistical_parity({k: v})), 3) for k,v in self.df_train.unprivileged_groups.items()]
        aod_list = [round(abs(metrics.average_odds({k: v})), 3) for k,v in self.df_train.unprivileged_groups.items()]
        eod_list = [round(abs(metrics.equal_opportunity({k: v})), 3) for k,v in self.df_train.unprivileged_groups.items()]
        spd = round(abs(metrics.wc_spd(list(self.df_train.unprivileged_groups.keys()))[-1]), 3)
        eod = round(abs(metrics.wc_eod(list(self.df_train.unprivileged_groups.keys()))[-1]), 3)
        aod = round(abs(metrics.wc_aod(list(self.df_train.unprivileged_groups.keys()))[-1]), 3)
        # spd, aod, eod = 0.2
