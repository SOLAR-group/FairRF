...
class NSGAIIoptimizer():    
    ...
    def __init__(self, dataset,round,model):
    ...
    def evaluateFitness(self, chrom):
        ...
        os.makedirs(f'outputs/{self.dataset}/fitnesses', exist_ok=True)
        with open(f'outputs/{self.dataset}/fitnesses/' + self.dataset + '_fitnesses.csv', 'a') as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', '-m', default='rf')
    ...
    if args.dataset == 'adult':
        ...
        design_dict = {'crime': ['black_people']}
    elif args.dataset == 'drug':
        design_dict = {'drug': ['race']}
    elif args.dataset == 'law':
        design_dict = {'law': ['race']}
    elif args.dataset == 'park':
        design_dict = {'park': ['sex']}
    elif args.dataset == 'cmc':
        design_dict = {'cmc': ['wife_religion']}
    else:
        design_dict = [
            # 'adult': ['sex', 'race'],
            'german' ,
    ...
    for dataset in design_dict:

            for run_num in range(1):
                nsga = NSGAIIoptimizer(dataset, run_num, args.model)
                pop, stats, non_dom = nsga.main()
                os.makedirs(f'outputs/{nsga.dataset}/logs', exist_ok=True) 
                pd.DataFrame(stats).to_csv(f'outputs/{nsga.dataset}/logs/' + nsga.dataset + '_stats_run_' + str(run_num) + '.csv', index=False)
                ...
                output_png_filename = nsga.dataset + '_' + nsga.obj_1 + '+' + nsga.obj_2 + '_run_' + str(run_num)
                ...
                os.makedirs(f'outputs/{nsga.dataset}', exist_ok=True)    
                with open(f"outputs/{nsga.dataset}/"+nsga.dataset + "_testset" + "_" + nsga.obj_1 + '+' + nsga.obj_2 + '_run_' + str(run_num) + '.txt', 'w') as f: 
